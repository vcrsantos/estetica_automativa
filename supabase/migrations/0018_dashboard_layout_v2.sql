-- POLIBRILHO — ajustes de layout do dashboard pra bater com o mockup: taxa
-- de retorno ganha uma janela anterior pra comparação (p.p. de variação), e
-- o comparativo entre unidades ganha taxa de retorno por unidade (ticket
-- médio já dá pra derivar no cliente a partir de faturamento/qtd_servicos,
-- sem precisar mudar o banco). create or replace é seguro de rodar de novo
-- — mesma assinatura da 0017, não precisa de drop.
...
begin
  return json_build_object(
    ...
    'comparativo_unidades', (
      ...
      from (
        select
          ...
          coalesce(fat.faturamento, 0) as faturamento,
          coalesce(fat.qtd_servicos, 0) as qtd_servicos,
          coalesce(ret.taxa_retorno_90d, 0) as taxa_retorno_90d
        from unidades u
        left join (
          select unidade_id, sum(valor_total) as faturamento, count(*) as qtd_servicos
          from ordens_servico
          where status <> 'cancelado' and entrada_em >= p_inicio and entrada_em < p_fim
          group by unidade_id
        ) fat on fat.unidade_id = u.id
        left join (
          select
            unidade_id,
            case when count(*) filter (where qtd >= 1) = 0 then 0
              else round(100.0 * count(*) filter (where qtd >= 2) / count(*) filter (where qtd >= 1), 1) end
              as taxa_retorno_90d
          from (
            select unidade_id, cliente_id, count(*) as qtd
            from ordens_servico
            where status <> 'cancelado' and entrada_em >= now() - interval '90 days'
            group by unidade_id, cliente_id
          ) visitas
          group by unidade_id
        ) ret on ret.unidade_id = u.id
        where u.ativo
      ) t
    ),
    'top_clientes', (
    ...
    'taxa_retorno', (
      ...
      visitas_90d_anterior as (
        select cliente_id, count(*) as qtd
        from ordens_servico
        where status <> 'cancelado'
          and entrada_em >= p_inicio - interval '90 days'
      ...
      select json_build_object(
        ...
        'taxa_retorno_90d_anterior', (
          select case when count(*) filter (where qtd >= 1) = 0 then 0
            else round(100.0 * count(*) filter (where qtd >= 2) / count(*) filter (where qtd >= 1), 1) end
          from visitas_90d_anterior
        ),
