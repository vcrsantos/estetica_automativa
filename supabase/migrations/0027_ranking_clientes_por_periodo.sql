-- POLIBRILHO — "Ranking de clientes por valor gasto" passa a respeitar o
-- período selecionado no filtro global, em vez de somar o histórico
-- inteiro do cliente independente do período escolhido (decisão original
-- documentada, revertida a pedido: o usuário espera que o card mude junto
-- com o filtro, como os demais). Mudança aditiva: mesma assinatura de
-- dashboard_insights(uuid, timestamptz, timestamptz), resto do corpo
-- idêntico ao de 0024_dashboard_insights_taxa_retorno_base.sql — só
-- 'top_clientes' ganha o filtro de `entrada_em` que os outros blocos já
-- tinham.
