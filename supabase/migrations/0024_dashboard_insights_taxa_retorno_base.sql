-- POLIBRILHO — dashboard-mockup-melhorias.html: quando a base de clientes
-- na janela de 90 dias é pequena demais (< 10), a taxa de retorno em % não
-- significa nada por si só ("100%" de 1 cliente que voltou não é o mesmo
-- sinal que 100% de 50) — o KPI passa a mostrar "—" com a legenda
-- "base pequena: N clientes em 90 dias" nesse caso. Isso exige saber
-- quantos clientes distintos entraram na janela, então 'taxa_retorno' ganha
-- a chave 'clientes_base' (mesma contagem que já alimenta o cálculo da
-- taxa, só exposta). Mudança aditiva: mesma assinatura de
-- dashboard_insights(uuid, timestamptz, timestamptz), resto do corpo
-- idêntico ao de 0017_seletor_periodo_global.sql.
